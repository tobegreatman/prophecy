import type { LotteryDraw, NumberInsight, PredictionScheme } from '../types/lottery.js';
...
function buildNumberInsights(draws: LotteryDraw[], scope: 'red' | 'blue') {
  ...
  const insights: NumberInsight[] = counters.map((bucket, index) => {
...
function chunkByZone(numbers: number[]) {
  const zones = [0, 0, 0];
...
function createMetrics(redNumbers: number[], blueNumber: number, redInsights: NumberInsight[]) {
...
function pickBalancedNumbers(
  orderedPool: NumberInsight[],
  size: number,
  existing: number[] = [],
  preferredZoneOrder = [0, 1, 2]
) {
  const selected = [...existing];
  const zoneLimits = [2, 2, 2];

  const zoneIndexFor = (value: number) => {
    if (value <= 11) {
      return 0;
    }
    if (value <= 22) {
...
function createSchemes(redInsights: NumberInsight[], blueInsights: NumberInsight[]) {
  const hotReds = redInsights
    .filter((item) => item.label === 'hot')
    .sort((left, right) => right.score - left.score);
  const warmReds = redInsights
  ...
  const schemes: PredictionScheme[] = [
    {
      id: 'momentum-wave',
      title: '热号延续',
...
function buildPatternSnapshot(draws: LotteryDraw[]) {
  const trendSeries = draws.slice(0, 18).map((draw) => {
  ...
  const oddEvenDistribution = draws.reduce<Record<string, number>>((result, draw) => {
    const oddCount = draw.redNumbers.filter((number) => number % 2 === 1).length;
  ...
  const zoneDistribution = draws.reduce<Record<string, number>>((result, draw) => {
...
export async function getHistory(issueCount: number) {
  return fetchDrawNotices(issueCount);
}

export async function getDashboard(issueCount: number) {
